...
async function sendOTPEmail(to, code, type) {
  ...
  // PORT 465 with SSL - works on Railway (587 is often blocked)
...
router.post('/send-otp', async function(req, res) {
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      console.error('EMAIL_USER or EMAIL_PASS missing in Railway variables');
      return res.status(500).json({ success: false, message: 'Email not configured. Add EMAIL_USER and EMAIL_PASS in Railway.' });
    }
    ...
    console.error('send-otp error:', err.message, '| code:', err.code);

    if (err.code === 'EAUTH' || (err.message && err.message.includes('Invalid login'))) {
      return res.status(500).json({ success: false, message: 'Gmail login failed. Check your App Password in Railway variables.' });
    }
    if (err.code === 'ETIMEDOUT' || err.code === 'ECONNECTION' || err.code === 'ESOCKET') {
      return res.status(500).json({ success: false, message: 'Email connection failed. Try again in a moment.' });
    }
